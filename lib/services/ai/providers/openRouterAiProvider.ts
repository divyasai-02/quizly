...
async function callOpenRouter<T>(prompt: string, apiKey: string, model: string) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`OpenRouter request failed with status ${response.status}${body ? `: ${body.slice(0, 200)}` : ""}`);
    }
...
export class OpenRouterAiProvider implements AiProvider {
  ...
  async generateQuizDraft(input: AiQuizGenerationInput) {
    ...
    const data = await callOpenRouter<AiProviderQuizDraftPayload>(buildQuizGenerationPrompt(normalized), this.apiKey, this.model);
  ...
  async generateRemedialQuiz(input: AiQuizGenerationInput) {
    const { normalized, warnings } = sanitizeAiQuizGenerationInput({ ...input, mode: "analytics-remedial", difficulty: input.difficulty ?? "Easy", questionCount: input.questionCount ?? 5, tone: input.tone ?? "Exam-focused" });
    const data = await callOpenRouter<AiProviderQuizDraftPayload>(buildRemedialQuizPrompt(normalized), this.apiKey, this.model);
    return this.wrap({ ...data, warnings: [...warningsArray(data.warnings), ...warnings] });
  }

  async regenerateQuestion(input: AiQuizGenerationInput & { questionIndex?: number }) {
    const { normalized, warnings } = sanitizeAiQuizGenerationInput({ ...input, questionCount: Math.max(1, (input.questionIndex ?? 0) + 1) });
    const data = await callOpenRouter<AiProviderQuizDraftPayload>(buildQuestionRegenerationPrompt({ ...normalized, questionIndex: input.questionIndex }), this.apiKey, this.model);
    return this.wrap({ ...data, warnings: [...warningsArray(data.warnings), ...warnings] });
  }

  async improveQuestion(input: { text?: string; tone?: string; userId?: string }) {
    if (!input.text?.trim()) throw new Error("Question text is required for improvement.");
    return this.wrap(await callOpenRouter<AiProviderQuestionImprovementPayload>(buildQuestionImprovementPrompt({ text: input.text.trim(), tone: input.tone ?? "Conceptual" }), this.apiKey, this.model));
  }

  async generateExplanation(input: { question?: string; answer?: string; userId?: string }) {
    if (!input.question?.trim()) throw new Error("Question text is required to generate an explanation.");
    return this.wrap(await callOpenRouter<AiProviderExplanationPayload>(buildExplanationPrompt({ question: input.question.trim(), answer: input.answer?.trim() }), this.apiKey, this.model));
  }

  private wrap<T>(data: T): AiProviderExecution<T> {
    return { data, provider: { provider: "openrouter", model: this.model } };
  }
}
