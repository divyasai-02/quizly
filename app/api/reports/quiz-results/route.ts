...
import { buildQuizResultsCsv, csvTableToString, loadReportingDataset, type ReportFilters } from "@/lib/services/reportingService";
...
export async function GET(request: Request) {
  try {
    ...
    const csv = buildQuizResultsCsv(dataset, getFilters(request));
    const { searchParams } = new URL(request.url);
    ...
    if (searchParams.get("format") === "csv") {
      await createNotification({
        ...
        title: "Quiz results report exported",
        message: `Your quiz results CSV export is ready: ${csv.fileName}.`,
        actionUrl: "/professor/reports"
      }).catch((error) => {
        console.error("[quizly-notifications] failed to create quiz-results report notification", error);
      });
